...
read IP
/sbin/ip -4 -o a | cut -d ' ' -f 2,7 | cut -d '/' -f 1 | grep enp3s0
if [ $? == 0 ]; then
  ETH0=enp3s0
  else
...
mkdir /opt/network/ && cp /opt/network/internetsw.sh && chmod +x /opt/network/internetsw.sh && ln -s /opt/network/internetsw.sh /usr/bin/internetsw  
